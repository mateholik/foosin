...
export function BestTeammates({ rows }: BestTeammatesProps) {
  return (
    <section className="brut-panel space-y-4">
      <h2 className="text-xl font-semibold tracking-tight text-slate-100 sm:text-2xl">Best Teammates</h2>
      {rows.length === 0 ? (
        <p className="text-sm text-slate-300">No teammate trends yet.</p>
      ) : (
          ...
          {rows.map((row, index) => (
            ...
            >
              <div>
                ...
                <p className="mt-1 text-base font-semibold text-slate-100">{row.team}</p>
              </div>
